...
$conn->close();
          ?>
          <!--  <div id="coffeeCart" class="cCart" ondrop="drop(event)" ondragover="allowDrop(event)"> -->
            </div>
          </div>
        </div>
          ...
          if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
            if($_POST["distance"] == "")
            {
              ...
              if ($conn->connect_error) {die("Connection failed: " . $conn->connect_error);}
                ...
                $dist = $_POST['distance'];
                $tm = $_POST['time'];
                $sql = "INSERT INTO ITEMS (PRODUCTID, USERID, SRC, DEST, DIST, TM) VALUES ('$iid', '$uid', '$src', '$dest', '$dist', '$tm')";
